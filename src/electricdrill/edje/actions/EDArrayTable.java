...
class EDArrayTable implements Cloneable {

    private Object table = null;
    private static final int ARRAY_BOUNDARY = 8;

    static void writeArrayTable(ObjectOutputStream s, EDArrayTable table) throws IOException {
        Object keys[];

        if (table.equals(null) || (keys = table.getKeys(null)).equals(null)) {
            s.writeInt(0);
        } else {
            int validCount = 0;
    ...
    public Object clone() {
        EDArrayTable newEDArrayTable = new EDArrayTable();
        if (isArray()) {
            Object[] array = (Object[])table;
            for (int i = 0 ;i < array.length-1 ; i+=2) {
                newEDArrayTable.put(array[i], array[i+1]);
            }
        } else {
            ...
            while (keys.hasMoreElements()) {
                ...
                newEDArrayTable.put(o,tmp.get(o));
            }
        }
        return newEDArrayTable;
    }
